...
type PlayerStatus = "Active" | "Injured";
...
const sportInput = document.createElement("input") as HTMLInputElement;
...
function loadData(): void {
  ...
  if (data) {
    const parsed: PlayerModel[] = JSON.parse(data);
    players = parsed.map(
      (p) => new Player(p.id, p.name, p.sport, p.age, p.status),
    );
  }
}
loadData();
...
function editInfo(playerItem: Player): void {
...
function render(data: Player[]): void {
  list.innerHTML = "";
